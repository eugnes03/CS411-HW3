...
class BattleModel:
    """
    Model class for the Battle component of the MealMax application.
    ...
    def __init__(self):
        """
            Initializes an empty list of combatant meals
        """
        self.combatants: List[Meal] = []

    def battle(self) -> str:
        """
            Battle logic for this model, in which it verifies there are enough contestants and gets scores through silly rule,
            and declares a winner based off those scores.

            Args:
                Combatants 0 and 1 that battle

            Returns:
                The meal of the winner
    ...
    def clear_combatants(self):
        """
            Clears the list of combatants. Useful for resetting the battle.
        """
    ...
    def get_battle_score(self, combatant: Meal) -> float:
        """
            Calculates a battle score for a given combatant based on the price, cuisine, and difficulty.

            Args:
                combatant (Meal): The combatant to calculate the battle score for.

            Returns:
                score = (price * len(cuisine)) - difficulty_modifier[difficulty] - value calculated and used in the battle function
        """
    ...
    def get_combatants(self) -> List[Meal]:
        """
            ...
            Returns:
                List[Meal]: Retrieve the current list of combatants
    ...
    def prep_combatant(self, combatant_data: Meal):
        """
            ...
            Args:
                combatant_data (Meal): The combatant to add to the list of combatants.

            Raises:
                ValueError: If the combatants list is full and cannot add more combatants.
